...
//      break;  <-- mistake ?
  ...
  // console.log('HCONF: ', HCONF, 'POT_LEVEL: ', POT_LEVEL, 'BET_LEVEL: ', BET_LEVEL, 'CALL_LEVEL: ', CALL_LEVEL, 'ID_CONF: ', ID_CONF);
  if (HCONF > 99) {
    if (POT_LEVEL > 75) return internal_what_do_x("60:ALLIN,10:BIG,20:MED,5:SMALL,5:CALL");
    if (num_players_playing_the_hand < 4) return internal_what_do_x("2:CALL,15:BIG,33:MED,50:SMALL");
  ...
  if (HCONF > 90) {
    ...
    return internal_what_do_x("2:ALLIN,6:CALL,15:MED,55:SMALL,22:BIG");
  }
  if (HCONF > 80) {
    if (POT_LEVEL > 50) {
    ...
    return internal_what_do_x("5:ALLIN,15:CALL,15:BIG,30:MED,35:SMALL");
  }
  ...
  if (HCONF > 60) {
    ...
    return internal_what_do_x("1:ALLIN,2:CALL,7:BIG,40:MED,50:SMALL");
  }
  if (HCONF > 50) {
    if (POT_LEVEL > 75) {
    ...
    return internal_what_do_x("1:ALLIN,1:BIG,5:CALL,20:MED,73:SMALL");
  }
  if (HCONF > 40) {
    if (BET_LEVEL > 40) {
      if (CALL_LEVEL < 40) return CALL;
    ...
    return internal_what_do_x("1:ALLIN,1:BIG,19:CALL,79:SMALL");
  }
  if (HCONF > 30) {
    if (BET_LEVEL > 40) {
      if (CALL_LEVEL < 30) return CALL;
    ...
    return internal_what_do_x("1:ALLIN,1:BIG,69:SMALL,29:CALL");
  }
  if (HCONF > 20) {
    if (BET_LEVEL > 30) {
      if (CALL_LEVEL < 30) return CALL;
    ...
    return internal_what_do_x("1:ALLIN,34:CALL,65:SMALL");
  }
  ...
  return internal_what_do_x("45:CALL,55:SMALL");
}
var hole_rankings =
...
function internal_setup () {
  ...
  BANKROLL = P.bankroll;
  // console.log('HCONF, CALL_LEVEL, BET_LEVEL, POT_LEVEL, BANKROLL: ', HCONF, CALL_LEVEL, BET_LEVEL, POT_LEVEL, BANKROLL);
  var total_bankrolls = get_pot_size();
  var current_pot = get_pot_size();
  ...
  SMALL = internal_round100(current_pot / 2);
  if (POT_LEVEL > 40) SMALL += 100;
  if (number_of_players_in_game > 3) {
    MED = CALL + BIG_BLIND * 4;
    BIG = CALL + BIG_BLIND * 10;
  } else {
    // SMALL += 100;
    // MED = internal_round100(CALL + 0.1 * BANKROLL); // consider minimum raise!
    // BIG = internal_round100(CALL + 0.2 * BANKROLL); // consider minimum raise!
    MED = internal_round100(current_pot);
    BIG = internal_round100(current_pot * 1.5);
  }
  ALLIN = BANKROLL;
...
//  console.log('Bot:    b:', b, 'P.bankroll: ', P.bankroll, 'size: ', size);
  if (size <= 0.015 || b <= 200) return 5;
  if (size <= 0.02 || b <= 400) return 10;
  if (size <= 0.03 || b <= 600) return 20;
  if (size <= 0.06 || b <= 1200) return 30;
  if (size <= 0.12 || b <= 2400) return 40;
  if (size <= 0.21 || b <= 4000) return 50;
  if (size <= 0.35 || b <= 6000) return 70;
  if (size <= 0.41 || b <= 8000) return 80;
  return 100;
}
