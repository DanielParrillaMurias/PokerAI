...
var tests = ["straight_flush", "four_of_a_kind", 
...
function test_full_house (player) {
  ...
  for (i = 0; i < my_cards.length; i++) {
    ranks[get_rank(my_cards[i]) - 2]++; //2=0  J=9 A=12
  }
  var three = "";
  var two = "";
  for (i = 0; i < 13; i++) {
  ...
  var num_needed = 5;
  var major_rank = "";          // rank del trío
  var num_mine_major = 0;       // cartas propias aportadas al trío
  if (three) {
    num_needed -= 3;
    major_rank = three;
  ...
  var minor_rank = "";          // rank de la pareja
  var num_mine_minor = 0;       // cartas propias aportadas a la pareja 
  if (two) {
    num_needed -= 2;
    minor_rank = two;
...
function get_player_cards (player) {
  var c = new Array(2);
  
  c[0] = player.carda;
  c[1] = player.cardb;
  return getCardNumbers(c);
}

function get_flop_cards (board) {
  var c = new Array(3);
...
function get_river_cards (board) {
  var c = new Array(5);
